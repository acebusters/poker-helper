...
describe('receipt + signer ', () => {
  ...
  it('should allow to sign and parse wait receipt.', (done) => {
    const waitReceipt = 'GoYP.yA/FWxvqrfbZPmT+68f+Q7x/p5oJ3wvXe2G5BxKLo1U=.OXW9RqJyLPIFyMVREs9bkAy2YXbh3Eu1s0O58yA5hXo=.G1j5n9EAESIzRFVmd4iZqrvM3e7/ABEiMwAAAAAAAAA=';
    const tableAddr = '0x00112233445566778899aabbccddeeff00112233';
    const created = 1492754385;
    const wait = new Receipt(tableAddr).wait(created);
    // test signing
    expect(wait.sign(PRIV)).to.eql(waitReceipt);
    // test parse
    const waitParams = [
      '0xc80fc55b1beaadf6d93e64feebc7fe43bc7fa79a09df0bd77b61b907128ba355',
      '0x3975bd46a2722cf205c8c55112cf5b900cb66176e1dc4bb5b343b9f32039857a',
      '0x1b58f99fd100112233445566778899aabbccddeeff0011223300000000000000',
    ];
    expect(Receipt.parseToParams(waitReceipt)).to.eql(waitParams);
    expect(Receipt.parse(waitReceipt)).to.eql({
      created,
      tableAddr,
      signer: ADDR,
      type: Type.WAIT,
