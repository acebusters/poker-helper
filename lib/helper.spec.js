...
describe('isActive()', () => {
  ...
  describe('TRUE when', () => {
    ...
    it('SITOUT receipt before the hand', () => {
      const sitoutPlayer = { address: P1_ADDR, last: new Receipt(TABLE_ADDR).sitOut(1, bignum(0)).sign(P1_PRIV) };
    ...
    it('BET receipt during the hand', () => {
      const leavingPlayer = { address: P1_ADDR, last: new Receipt(TABLE_ADDR).bet(1, bignum(200)).sign(P1_PRIV) };
      ['preflop', 'flop', 'turn', 'river', 'showdown'].forEach(state => {
  ...
  describe('FALSE when', () => {
    ...
    it('FOLD receipt', () => {
      const foldedPlayer = { address: P1_ADDR, last: new Receipt(TABLE_ADDR).fold(1, bignum(500)).sign(P1_PRIV) };
      forEachState(state => expect(isActive(foldedPlayer, state, rc)).to.be.false);
    });

    it('SHOW receipt', () => {
      const showdownPlayer = { address: P1_ADDR, last: new Receipt(TABLE_ADDR).show(1, bignum(200)).sign(P1_PRIV) };
