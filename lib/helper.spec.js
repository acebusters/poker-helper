...
describe('findMaxRaise() ', () => {
  it('should find the max raise when there was raise', (done) => {
    ...
    expect(pokerHelper.findMaxRaise(lineup, 2, 100)).to.equal(300);
    done();
  });
  ...
  it('should find the max raise when there was re-raise', (done) => {
    ...
    expect(pokerHelper.findMaxRaise(lineup, 3, 100)).to.equal(700);
    done();
  });
  ...
  it('should return betting amount when there was no raise', (done) => {
    ...
    expect(pokerHelper.findMaxRaise(lineup, 2, 100)).to.equal(100);
    done();
  });

  it('should return 0 when last round max == maxbet', (done) => {
    let lineup = [{
        address: P1_ADDR,
        last: new EWT(ABI_BET).bet(1, 100).sign(P1_KEY)
      }, {
    ...
    expect(pokerHelper.findMaxRaise(lineup, 2, 100)).to.equal(0);
    done();
  });
});