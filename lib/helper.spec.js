...
import PokerHelper, { isActive } from './helper';
...
describe('countActivePlayers() ', () => {
  ...
  it('should notice undefined seats.', () => {
    ...
    expect(pokerHelper.countActivePlayers(hand.lineup, hand.state)).to.equal(2);
  });
});
...
describe('calcRakeSize', () => {
  it('should return zero for preflop.', () => {
    expect(pokerHelper.calcRakeSize([{
      address: P_EMPTY,
    }, {
      address: P1_ADDR,
      ...
      address: P2_ADDR,
      last: new Receipt(TABLE_ADDR).bet(1, bignum(1000)).sign(P2_PRIV),
    }], bignum(10), 'preflop')).to.eql(0);
  });

  it('should calculate rake size.', () => {
    expect(pokerHelper.calcRakeSize([{
      address: P1_ADDR,
    ...
    expect(pokerHelper.calcRakeSize([{
      ...
      address: P_EMPTY,
    }], bignum(1600), 'showdown')).to.eql(10);
  });
});
...
describe('calcDistribution', () => {
  ...
  it('should return null for undefined distribution.', () => {
    expect(pokerHelper.calcDistribution([{
      address: P1_ADDR,
    ...
    }], 'river', null, 10, P_EMPTY)).to.eql(null);
  });
...
const forEachState = (cb) => {
  ['waiting', 'dealing', 'preflop', 'flop', 'turn', 'river', 'showdown'].forEach(cb);
};

describe('isActive()', () => {
  const rc = new ReceiptCache();

  it('should throw if player is null or undefined', () => {
    expect(isActive.bind(this, null, 'waiting', rc)).to.throw('player undefined');
    expect(isActive.bind(this, undefined, 'waiting', rc)).to.throw('player undefined');
  });

  it('should throw if player has no receipt during the hand', () => {
    const noReceiptPlayer = { address: P1_ADDR };

    ['preflop', 'flop', 'turn', 'river', 'showdown'].forEach((state) => {
      expect(isActive.bind(this, noReceiptPlayer, state, rc)).to.throw('receipt undefined');
    });
  });

  describe('TRUE when', () => {
    it('all-in on showdown', () => {
      expect(isActive({ address: P1_ADDR, sitout: 'allin' }, 'showdown', rc)).to.be.true;
    });

    it('no receipt in WAITING state', () => {
      expect(isActive({ address: P1_ADDR }, 'waiting', rc)).to.be.true;
    });

    it('no receipt in DEALING state', () => {
      expect(isActive({ address: P1_ADDR }, 'dealing', rc)).to.be.true;
    });

    it('SITOUT receipt before the hand', () => {
      const sitoutPlayer = {
        address: P1_ADDR,
        last: new Receipt(TABLE_ADDR).sitOut(1, bignum(0)).sign(P1_PRIV),
      };
      expect(isActive(sitoutPlayer, 'waiting', rc)).to.be.true;
      expect(isActive(sitoutPlayer, 'dealing', rc)).to.be.true;
    });

    it('BET receipt during the hand', () => {
      const leavingPlayer = {
        address: P1_ADDR,
        last: new Receipt(TABLE_ADDR).bet(1, bignum(200)).sign(P1_PRIV),
      };
      ['preflop', 'flop', 'turn', 'river', 'showdown'].forEach((state) => {
        expect(isActive(leavingPlayer, state, rc)).to.be.true;
      });
    });
  });
  ...
  describe('FALSE when', () => {
    it('sitout', () => {
      const sitoutPlayer = { address: P1_ADDR, sitout: Date.now() };

      forEachState(state => expect(isActive(sitoutPlayer, state, rc)).to.be.false);
    });

    it('no address', () => {
      const noAddressPlayer = { };

      forEachState(state => expect(isActive(noAddressPlayer, state, rc)).to.be.false);
    });

    it('empty address', () => {
      const emptyAddressPlayer = { address: P_EMPTY };

      forEachState(state => expect(isActive(emptyAddressPlayer, state, rc)).to.be.false);
    });
    ...
    it('FOLD receipt', () => {
      ...
      forEachState(state => expect(isActive(foldedPlayer, state, rc)).to.be.false);
    });

    it('SHOW receipt', () => {
      const showdownPlayer = {
        address: P1_ADDR,
        last: new Receipt(TABLE_ADDR).show(1, bignum(200)).sign(P1_PRIV),
      };
      forEachState(state => expect(isActive(showdownPlayer, state, rc)).to.be.false);
    });

    it('SITOUT receipt during the hand', () => {
      const sitoutPlayer = {
        address: P1_ADDR,
        last: new Receipt(TABLE_ADDR).sitOut(1, bignum(0)).sign(P1_PRIV),
      };
      ['preflop', 'flop', 'turn', 'river', 'showdown'].forEach((state) => {
        expect(isActive(sitoutPlayer, state, rc)).to.be.false;
      });
    });
  });
});
