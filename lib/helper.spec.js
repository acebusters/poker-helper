...
describe('whosTurn in River', () => {
  ...
  it('premature caller', (done) => {
    let hand = {
      state: 'river',
      lineup: [{
          address: P1_ADDR,
          last: new EWT(ABI_BET).bet(1, 20).sign(P1_KEY)
        }, {
          address: P2_ADDR,
          last: new EWT(ABI_BET).bet(1, 40).sign(P2_KEY)
        }, {
          address: P3_ADDR,
...
describe('check for next hand', () => {
  ...
  it('should not advance to next hand because not all active players have shown their hand', (done) => {
    ...
    expect(pokerHelper.checkForNextRound(hand)).to.equal(false);
    done();
  });
  ...
  it('should advance to next hand because all active players have shown their hand', (done) => {
    ...
    expect(pokerHelper.checkForNextRound(hand)).to.equal(true);
    done();
  });
