...
describe('isHandComplete() ', () => {
  ...
  it('should allow to find hand complete with 1 player left', () => {
    const lineup = [{
      address: P1_ADDR,
      last: new EWT(ABI_SIT_OUT).sitOut(3, 50).sign(P1_PRIV),
      sitout: 1,
    }, {
      address: P_EMPTY,
    }, {
      address: P3_ADDR,
      last: new EWT(ABI_BET).bet(3, 100).sign(P3_PRIV),
    }, {
      address: P_EMPTY,
    }];
...
describe('isBettingDone() ', () => {
  ...
  it('should proceed to showdown when second to last called-in.', () => {
    const lineup = [{
      address: P1_ADDR, // BU
      last: new EWT(ABI_BET).bet(1, 1200).sign(P1_PRIV),
    }, {
      address: P_EMPTY,
    }, {
      address: P2_ADDR, // SB
    ...
    expect(pokerHelper.isBettingDone(lineup, 0, 'flop', 200)).to.equal(true);
  });
  ...
  it('should not proceed to next round when all-in needs to be called from the bb.', () => {
    ...
    expect(pokerHelper.isBettingDone(lineup, 3, 'preflop', 50)).to.equal(false);
  });
  ...
  it('should proceed to next round when all-in called by all.', () => {
    ...
    const lineup = [{
      ...
      address: P1_ADDR,
      last: new EWT(ABI_BET).bet(1, 800).sign(P1_PRIV), // SB
      sitout: 'allin',
    }, {
      address: P2_ADDR,
      last: new EWT(ABI_BET).bet(1, 1200).sign(P2_PRIV), // BB
    }, {
      address: P3_ADDR,
      last: new EWT(ABI_BET).bet(1, 1200).sign(P3_PRIV), // BU
    }];
  ...
  it('should proceed to next round when two-way all-in.', () => {
    ...
    const lineup = [{
      ...
      address: P2_ADDR,
      last: new EWT(ABI_BET).bet(1, 1100).sign(P2_PRIV), // BB
      sitout: 'allin',
    }, {
      address: P3_ADDR,
      last: new EWT(ABI_BET).bet(1, 1200).sign(P3_PRIV), // BU
