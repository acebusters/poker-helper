...
describe('whosTurn() in state PreFlop ', () => {
  ...
  it('should find BU when BB payed.', () => {
    ...
    expect(pokerHelper.getWhosTurn(lineup, 2, 'preflop', 100)).to.equal(2);
  });
  ...
  it('should find SB when BU calls preflop.', () => {
    const lineup = [{
      ...
      address: P2_ADDR, // SB
      last: new EWT(ABI_BET).bet(1, 25).sign(P2_PRIV),
    }, {
      address: P3_ADDR, // BB
      last: new EWT(ABI_BET).bet(1, 50).sign(P3_PRIV),
    }];
    expect(pokerHelper.getWhosTurn(lineup, 1, 'preflop', 50)).to.equal(2);
  });
  ...
  it('should find turn of UTG+1.', () => {
    const hand = {
      ...
      lineup: [{
        address: P1_ADDR,
        last: new EWT(ABI_BET).bet(1, 50).sign(P1_PRIV), // BB
      }, {
        address: P2_ADDR,
        last: new EWT(ABI_BET).bet(1, 0).sign(P2_PRIV), // BU
      }, {
        address: P3_ADDR,
        last: new EWT(ABI_BET).bet(1, 50).sign(P3_PRIV), // SB
      }],
      dealer: 1,
    };
  ...
  it('should find correct pos with overcaller', () => {
    const hand = {
      ...
      lineup: [{
        address: P1_ADDR,
        last: new EWT(ABI_BET).bet(1, 50).sign(P1_PRIV), // UTG
      }, {
        address: P2_ADDR,
        last: new EWT(ABI_BET).bet(1, 50).sign(P2_PRIV), // BU
      }, {
        address: P3_ADDR,
        last: new EWT(ABI_BET).bet(1, 25).sign(P3_PRIV), // SB
      }, {
        address: P4_ADDR,
        last: new EWT(ABI_BET).bet(1, 50).sign(P4_PRIV), // BB
      }],
      dealer: 1,
    };
    expect(pokerHelper.getWhosTurn(hand.lineup, hand.dealer, hand.state, 50)).to.eql(2);
  ...
  it('should find turn after all-in.', () => {
    ...
    const lineup = [{
        address: P1_ADDR,
        last: new EWT(ABI_BET).bet(4, 100).sign(P1_PRIV),
      }, {
        address: P_EMPTY,
      }, {
        address: P3_ADDR,
        last: new EWT(ABI_BET).bet(4, 800).sign(P3_PRIV),
        sitout: 'allin',
      }, {
        address: P4_ADDR,
        sitout: 1,
      }];
    expect(pokerHelper.getWhosTurn(lineup, dealer, state, 100)).to.eql(0);
  });
