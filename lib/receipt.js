  ...
  */
  ...
  static parseToParams(receipt) {
    ...
    if (bufs.type === Type.SETTLE) {
      ...
      return [`0x${bufs.parts[2].toString('hex')}`, `0x${bufs.parts[3].toString('hex')}`, `0x${v.toString(16)}${bufs.parts[0].toString('hex')}${bufs.parts[1].toString('hex')}`];
    }
  ...
  static parse(receipt) {
    ...
    switch (bufs.type) {
      ...
      case Type.BET: {
        ...
        rv.amount = new BigNumber(bufs.parts[2].readUIntBE(25, 6)).mul(jozDecimals);
        break;
      }
      case Type.RAKE_REQ: {
        rv.handId = bufs.parts[2].readUInt32BE(21);
