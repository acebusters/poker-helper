 ...
 * @returns number of active players
 */
const getMaxActive = function(lineup, state, isActiveLineup, dealer, bbAmount, bbPos, rc) {
  let max = 0;
  let pos = -1;
...
PokerHelper.prototype.getWhosTurn = function getWhosTurn(lineup, dealer, state, bbAmount) {
  ...
  let maxBet;
  try {
    maxBet = getMaxActive(activePlayers, state, true, dealer, bbAmount, bbPos, this.rc);
  } catch(e) {
    maxBet = null;
  }
