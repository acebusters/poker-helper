...
export default class PokerHelper {
  ...
  calcRakeSize(lineup, sb, state) {
    if (state === 'waiting' || state === 'dealing' || state === 'preflop') {
      return 0;
    }

    const playersCount = lineup.filter((_, i) => wasInvolved(lineup, i, state)).length;
    const prcIndex = Math.min(Math.ceil(playersCount / 2), 3);
    /* eslint-disable no-multi-spaces */
    const rakePerMilScale = [
      [babz(2),     45, 45, 45],
      [babz(100),   20, 30, 40],
      [babz(200),   20, 30, 30],
      [babz(1000),  20, 25, 25],
    ...
    return rakePerMilScale.reduce((memo, row) => {
      if (memo[0].gt(sb)) {
        return memo;
      }

      return row;
    }, [babz(0), 0, 0, 0])[prcIndex];
  }
