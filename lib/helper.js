...
const checkForReceipt = (lineup, type, rc) => {
  for (let pos = 0; pos < lineup.length; pos += 1) {
    const receipt = (lineup[pos].last) ? rc.get(lineup[pos].last).type : '';
    if (receipt === type) {
      return true;
    }
  }
...
const countReceipts = (lineup, type, rc) => {
  ...
  let counter = 0;
  for (let pos = 0; pos < lineup.length; pos += 1) {
    const receipt = (lineup[pos].last) ? rc.get(lineup[pos].last) : null;
...
const maxAllIn = (lineup, rc) => {
  ...
  for (let pos = 0; pos < lineup.length; pos += 1) {
    if (lineup[pos].sitout && lineup[pos].sitout === 'allin') {
      const amount = (lineup[pos].last) ? rc.get(lineup[pos].last).amount.toNumber() : 0;
...
export default class PokerHelper {
  ...
  getMaxBet(lineup, state) {
    ...
    for (let i = startPos; i < lineup.length + startPos; i += 1) {
      ...
      const last = lineup[relPos].last ? this.rc.get(lineup[relPos].last) : null;
      const amount = (last && last.type !== Type.SIT_OUT) ? last.amount.toNumber() : 0;
  ...
  calculatePotsize(lineup) {
    ...
    for (let i = 0; i < lineup.length; i += 1) {
      ...
      potSize += (receipt) ? receipt.amount.toNumber() : 0;
    }
    return potSize;
  }
  ...
  getWhosTurn(lineup, dealer, state, bbAmount) {
    ...
    const checkType = (state === 'preflop') ? // eslint-disable-line
        ...
        Type.CHECK_FLOP : (state === 'turn') ? // eslint-disable-line
        Type.CHECK_TURN : (state === 'river') ?
        Type.CHECK_RIVER : 0;
  ...
  calcDistribution(
    ...
    // create pots
    for (let i = 0; i < lineup.length; i += 1) {
      const last = (lineup[i].last) ? this.rc.get(lineup[i].last) : null;

      if (last) {
        const amount = last.amount.toNumber();

        if (
          (state === 'showdown' && last.type === Type.SHOW) ||
          (state !== 'showdown' && (this.isActivePlayer(lineup, i) || lineup[i].sitout === 'allin'))
        ) {
          if (!pots.includes(amount)) {
            pots.push(amount);
          }
          players.push({ pos: i, active: true, amount });
        } else {
          players.push({ pos: i, active: false, amount });
        }
      }
    }
    ...
    if (pots.reduce((a, b) => a + b, 0) === 0) {
      ...
      return players.reduce((result, player) => ({
        ...result,
        [lineup[player.pos].address]: player.amount - ((player.amount / 1000) * rakePerMil),
        [rakeAddr]: result[rakeAddr] + ((player.amount / 1000) * rakePerMil),
      }), { [rakeAddr]: 0 });
    }
