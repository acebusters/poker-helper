 ...
 * @returns
 */
const findSbPos = (lineup, dealer, state, type, rc) => {
  ...
  return findNextPlayer(lineup, dealer + 1, lineupType, state, rc);
};
 ...
 * @returns
 */
const findBbPos = (lineup, dealer, state, type, rc) => findNextPlayer(
  lineup,
  findSbPos(lineup, dealer, state, type, rc) + 1,
  type === 'tournament' ? 'busy' : 'involved',
  state,
  rc,
);
...
export default class PokerHelper {
  ...
  getSbPos(lineup, dealer, state, type = 'cashgame') {
    return findSbPos(lineup, dealer, state, type, this.rc);
  }
