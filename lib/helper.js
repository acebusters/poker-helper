...
PokerHelper.prototype.whosTurn = function(hand) {
  ...
  if (hand.state === 'showdown') {
    let breakCond = hand.lineup.length;
    let nextPos = hand.dealer;
    // itterate through lineup, find first guy without show receipt
    while (breakCond > 0) {
      const lastAction = (hand.lineup[nextPos].last) ? this.rc.get(hand.lineup[nextPos].last).abi[0].name : '';
      if (lastAction.indexOf('show') > -1) {
        nextPos = this.nextPlayer(hand.lineup, hand.dealer, 'involved');
        breakCond--;
      } else {
        breakCond = 0;
      }
