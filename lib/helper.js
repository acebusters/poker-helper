...
PokerHelper.prototype.getActiveLineup = function(lineup, dealer, state) {
  const activeLineup = [];
  const startPos = this.nextActivePlayer(lineup, dealer + 1);
...
PokerHelper.prototype.whosTurn = function(hand) {
  ...
  if (activeLineup.length === 0 || (activeLineup.length === 1 && hand.state !== 'showdown')) {
    // no ones turn if only one or 0 player(s)
    return -1
  }

  if (!this.evenLineup(activeLineup)) {
    // if betting amounts are not even return next player of to the maxBettor
    return activeLineup[(this.getMaxBet(activeLineup, hand.state).pos + 1) % activeLineup.length].pos;
  } else {
...
//     let pos;
//     for (let i = hand.dealer + 1; i <= (hand.dealer + 1) + hand.lineup.length; i++) {
//       pos = i % hand.lineup.length;
//       if (this.isActivePlayer(hand.lineup) || hand.lineup[pos].sitout === 'allin') {
//         return pos;
//       }
//     }
//   }
