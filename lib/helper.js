 ...
 * @returns number of active players
 */
const getMaxActive = function(lineup, isActiveLineup, dealer, state, bbAmount, bbPos, rc) {
  let max = 0;
  let pos = -1;
  const activeLineup = (isActiveLineup) ? lineup : activeLineup(lineup, dealer, state, rc);
  activeLineup.forEach((player, index) => {
      if (!player.last) {
        return;
      }
      const amount = (player.last) ? rc.get(player.last).values[1] : 0;
...
PokerHelper.prototype.getWhosTurn = function getWhosTurn(lineup, dealer, state, bbAmount) {
  ...
  let maxBet;
  try {
    maxBet = getMaxActive(activePlayers, true, dealer, state, bbAmount, bbPos, this.rc);
  } catch(e) {
    maxBet = null;
  }
