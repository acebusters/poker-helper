 ...
 * @returns bool
 */
export const isBusy = (player) => {
  if (typeof player === 'undefined' || player === null) {
    throw new Error('player undefined');
  }

  // check seat state
  if (!player.address || player.address === EMPTY) {
    return false;
  }
...
const countPlayers = (lineup, type, state, rc) => {
  const filters = {
    active: pos => isActive(lineup[pos], state, rc),
    busy: pos => isBusy(lineup[pos], state, rc),
    involved: pos => wasInvolved(lineup, pos, state, rc),
  };

  return lineup.filter((_, pos) => filters[type](pos)).length;
};
...
const findNextPlayer = (lineup, startPos, type, state, rc) => {
  ...
  return shiftPos(
    lineup
      .findIndex((_, pos) => filter(shiftPos(pos))),
  );
};
