 ...
 */
const activeLineup = function(lineup, dealer, state, rc) {
  const activePlayers = [];
  try {
    let startPos;
    if (state !== 'preflop') {
      ...
      // BB is last to act
      let bbPos = findBbPos(lineup, dealer, state, rc);
 ...
 * @returns
 */
const findBbPos = function findBbPos(lineup, dealer, state, rc) {
 ...
 * @returns
 */
const findNextPlayer = function findNextPlayer(lineup, startPos, type, state, rc) {
  let newPos = startPos % lineup.length;
  for (let i = newPos; i < lineup.length + newPos; i += 1) {
    const pos = i % lineup.length;
    if (type === 'active') {
      if (isActive(lineup, pos, rc)) {
        newPos = pos;
        break;
      }
    } else if (type === 'involved') {
