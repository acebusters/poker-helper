...
/**
 * findNextPlayer finds next player in lineup
 *
 * @param lineup    lineup is an array of seats
 * @param startPos  position in lineup to start looking for next player
 * @param type      type of player we are looking for, 'involved' or 'active'
 * @param state     state of hand when looking
 * @param rc        receiptCache holding parsed receipts
 * @returns         index of next player in lineup
 */
const findNextPlayer = (lineup, startPos, type, state, rc) => {
  let newPos;
  if (startPos < 0) {
    newPos = lineup.length + startPos;
  }
  newPos = startPos % lineup.length;
...
/**
 * findPrevPlayer finds previous player in lineup
 *
 * @param lineup    lineup is an array of seats
 * @param startPos  position in lineup to start looking for previous player
 * @param type      type of player we are looking for, 'involved' or 'active'
 * @param state     state of hand when looking
 * @param rc        receiptCache holding parsed receipts
 * @returns         index of previous player in lineup
 */
const findPrevPlayer = (lineup, startPos, type, state, rc) => {
  let newPos;
  if (startPos < 0) {
    newPos = lineup.length + startPos;
  }
  newPos = startPos % lineup.length;
  for (let i = (lineup.length + newPos) - 1; i > newPos; i -= 1) {
    const pos = i % lineup.length;
    if (type === 'active') {
      if (isActive(lineup[pos], state, rc)) {
        return pos;
  ...
  try {
    ...
    if (state === 'preflop') {
      ...
      try {
        ...
        startPos = findNextPlayer(lineup, bbPos + 1, 'active', state, rc);
      } catch (err) {
    ...
    if (state !== 'preflop' || typeof startPos === 'undefined') {
      // BU is last to act
      startPos = findNextPlayer(lineup, dealer + 1, 'active', state, rc);
    }
  ...
  if (state !== 'waiting') {
    ...
    if (!evenLineup(activePlayers, this.rc)) {
      ...
      if (last && last.values[1] === maxBet.amount) {
        next.pos = findNextPlayer(lineup, next.pos + 1, 'active', state, this.rc);
      }
      return next.pos;
    }
  }
 ...
 * @returns
 */
PokerHelper.prototype.prevPlayer = function prevPlayer(lineup, startPos, type, state) {
  return findPrevPlayer(lineup, startPos, type, state, this.rc);
};
