...
PokerHelper.prototype.findMaxRaise = function(lineup, dealer, lastRoundMaxBet) { 
...
PokerHelper.prototype.getBbPos = function(lineup, dealer) {
  if (typeof dealer === 'undefined' || !lineup) {
    return -1;
  }
  if (dealer < 0 || dealer >= lineup.length || lineup.length < 2) {
  ...
  const nextNext = this.nextActivePlayer(lineup, next + 1);
  if (nextNext == next) {
    return -1;
  }
  return nextNext;
...
PokerHelper.prototype.allDone = function(lineup, dealer, handState, max, bbAmount) {
  ...
  if (handState !== 'preflop') {
    ...
    if (done && bbAmount) {
      const bbPos = this.getBbPos(lineup, dealer);
      const rec = this.rc.get(lineup[bbPos].last);
      // make sure the bb gets to check preflop.
      if (rec.values[1] == bbAmount && rec.abi[0].name.indexOf('check') < 0) {
        return false;
      }
    }
  }
