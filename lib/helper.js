...
PokerHelper.prototype.isTurn = function(lineup, dealer, state, bb, addr) {
...
PokerHelper.prototype.getTurn = function(lineup, dealer, state, bb) {
  throw new Error('not implemented');
}
...
PokerHelper.prototype.isHandComplete = function(lineup, dealer, state) {
  const activePlayerCount = this.getActiveLineup(lineup, dealer, state).length;
  const allInPlayerCount = this.countAllIns({ lineup });
  if (state == 'showdown') {
...
PokerHelper.prototype.renderPublicState = function(hand) {
  if (hand.state == 'showdown') {
