 ...
 * @returns
 */
PokerHelper.prototype.getMinRaiseAmount = function getMinRaiseAmount(lineup, state, dealer, lastRoundMaxBet, bbAmount) {
  if (!lineup || dealer === undefined) {
    throw new Error('invalid params.');
  }
  // TODO: pass correct state
  const bbPos = this.getBbPos(lineup, dealer, state);
  const max = this.getMaxFromActive(lineup, state, false, dealer, bbAmount, bbPos);
  ...
  for (let i = 0; i < lineup.length; i += 1) {
    const last = (lineup[i].last) ? this.rc.get(lineup[i].last).values[1] : 0;
...
/**
 * finds the maxBet for an active lineup
 *
 * @param active lineup
 * @param hand state
 * @param bb Amount
 * @param bb pos
 * @returns maxbet amount and position
 */
PokerHelper.prototype.getMaxFromActive = function(lineup, state, isActiveLineup, dealer, bbAmount, bbPos) {
  let max = 0;
  let pos = -1;
  const activePlayers = (isActiveLineup) ? lineup : activeLineup(lineup, dealer, state, this.rc);
  activePlayers.forEach((player, index) => {
    if (!player.last) {
      return;
    }
    const amount = (player.last) ? this.rc.get(player.last).values[1] : 0;
    // if we are in dealing the last 0 bet is the max bet
    const bbMaxBettor = (amount === bbAmount && player.pos === bbPos);
    if ((amount >= max || (amount === 0 && state === 'dealing')) && !bbMaxBettor) {
      max = amount;
      pos = index;
    }
  });
...
PokerHelper.prototype.getWhosTurn = function getWhosTurn(lineup, dealer, state, bbAmount) {
  ...
  let maxBet;
  try {
    maxBet = this.getMaxFromActive(activePlayers, state, true, dealer, bbAmount, bbPos);
  } catch(e) {
    maxBet = null;
  }
...
PokerHelper.prototype.isBettingDone = function isBettingDone(lineup, dealer, state, bbAmount) {
  ...
  let maxBet;
  try {
    maxBet = this.getMaxFromActive(lineup, state, true, dealer, bbAmount, bbPos);
  } catch (err) {
    if (state === 'waiting') {
      return false;
    }
 ...
 */
PokerHelper.prototype.getWinners = function getWinners(lineup, dealer, board, state) {
  if (!lineup || !state) {
    throw new Error('invalid params');
  }
...
PokerHelper.prototype.getMaxBet = function getMaxBet(lineup, state) {
  console.warn('deprecated: Please use getMaxFromActive(lineup, state, isActiveLineup, dealer, bbPos, bbAmount) instead');
  let max = 0;
  let pos = -1;
  for (let i = 0; i < lineup.length; i += 1) {
    if (!lineup[i].last) {
 ...
 * @param lineup
 * @param dealer
 * @param lastRoundMaxBet
 * @param state
 * @returns
 * @deprecated Since version 0.3.10. Please use isHandComplete(lineup, dealer, state) instead
 */
PokerHelper.prototype.findMinRaiseAmount = function findMinRaiseAmount(lineup, dealer, lastRoundMaxBet, state) {
  console.warn('deprecated: please use getMinRaiseAmount(lineup, state, dealer, lastRoundMaxBet, bbAmount) instead');
  if (!lineup || dealer === undefined) {
    throw new Error('invalid params.');
  }
  // TODO: pass correct state
