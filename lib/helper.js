...
PokerHelper.prototype.isBettingDone = function isBettingDone(lineup, dealer, state, bbAmount) {
  ...
  if (checker.last) {
    ...
    const checkerRec = this.rc.get(checker.last);
    const checkCount = countReceipts(lineup, checkerRec.type, this.rc);
...
PokerHelper.prototype.calcDistribution = function calcDistribution(
  ...
  for (i = 0; i < lineup.length; i += 1) {
    ...
    if (last) {
      ...
      if (state === 'showdown') {
        if (last.type === Type.SHOW) {
          if (!pots.includes(last.amount.toNumber())) {
            pots.push(last.amount.toNumber());
          }
          active = true;
        }
