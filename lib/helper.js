...
PokerHelper.prototype.whosTurn = function(hand) {
  ...
  if (allEven) {
    ...
    if (checkFound) {
      ...
      let prev = -1;
      while (breakCond > 0 && !this.hasPassedDealer(prev, nextPos, hand.dealer, hand.lineup.length)) {
...
PokerHelper.prototype.hasPassedDealer = function(prev, next, dealer, length) {
  if (prev == -1)
    return false;
  if (next > dealer && prev > dealer)
    return false;
