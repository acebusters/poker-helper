 ...
 *
 */
const getMaxFromActive = (activePlayers, state, dealer, bbAmount, bbPos, rc) => {
  let max = 0;
  let pos = -1;
...
PokerHelper.prototype.getWhosTurn = function getWhosTurn(lineup, dealer, state, bbAmount) {
  ...
  if (state !== 'waiting') {
    const bbPos = findBbPos(lineup, dealer, state, this.rc);
    maxBet = getMaxFromActive(activePlayers, state, dealer, bbAmount, bbPos, this.rc);
