 ...
 */
PokerHelper.prototype.activeLineup = function(lineup, dealer, state, rc) {
  const activePlayers = [];
  try {
    let startPos;
    if (state !== 'preflop') {
      ...
      // BB is last to act
      let bbPos = this.getBbPos(lineup, dealer, state);
...
PokerHelper.prototype.getWhosTurn = function getWhosTurn(lineup, dealer, state, bbAmount) {
  ...
  const activePlayers = this.activeLineup(lineup, dealer, state, this.rc);
  let maxBet;
