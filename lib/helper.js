...
PokerHelper.prototype.isBettingDone = function isBettingDone(lineup, dealer, state, bbAmount) {
  ...
  let checker = findLatest({ sorted: true, lineup: activePlayers }, 'check', this.rc);
  if (checker.last) {
    const lastRec = this.rc.get(lineup[activePlayers[activePlayers.length-1].pos].last);
    const checkRec = this.rc.get(lineup[checker.pos].last);
    if (lastRec.abi[0].name.toLowerCase() === `check${state}` &&
      checkRec.abi[0].name.toLowerCase() === `check${state}`) {
      return true;
    }
    if (checkRec.abi[0].name.toLowerCase() !== `check${state}` && state !== 'preflop') {
      return false;
    }
    if (checkRec.abi[0].name.toLowerCase() !== lastRec.abi[0].name.toLowerCase()) {
      return false;
    }
  }
  if (state === 'preflop') {
  ...
  for (i = 0; i < evals.length; i += 1) {
    for (j = 0; j < players.length; j += 1) {
      if (players[j].amount > 0) {
        let limit = (i > 0) ? evals[i].limit - evals[i-1].limit : evals[i].limit;
