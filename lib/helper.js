 ...
 * @returns bool
 */
export const isActive = (seat, state, rc) => {
  ...
  // get last receipt
  const receipt = seat.last && rc.get(seat.last);
  if (!receipt || !receipt.type || !receipt.amount) {
    ...
    return !(
      receipt.type === Type.FOLD ||
      receipt.type === Type.SHOW ||
      (receipt.type === Type.SIT_OUT && state !== 'waiting' && state !== 'dealing')
    );
  }
};
...
const countPlayers = (lineup, type, state, rc) => (
  lineup
    .filter(seat => lineupFilters[type](seat, state, rc))
    .length
);
...
const findNextPlayer = (lineup, startPos, type, state, rc) => {
  const shiftPos = pos => (startPos + pos) % lineup.length;
  const filter = lineupFilters[type];
 ...
 * @returns
 */
const findBbPos = (lineup, dealer, state, type, rc) => {
  ...
  const lineupType = type === 'tournament' ? 'busy' : 'involved';

  const next = findNextPlayer(lineup, dealer + 1, lineupType, state, rc);
  if (next === dealer) {
    throw new Error('only one player found.');
  }
  const activePlayerCount = countPlayers(lineup, lineupType, state, rc);
  if (activePlayerCount === 2) {
    return next;
  }
  const nextNext = findNextPlayer(lineup, next + 1, lineupType, state, rc);
  if (nextNext === next) {
    throw new Error('only one player found.');
  }
  return nextNext;
...
const countReceipts = (lineup, type, rc) => {
  ...
  return lineup.filter(seat => seat.last && rc.get(seat.last).type === type).length;
};
...
export default class PokerHelper {
   ...
   * @returns
   */
  getSbPos(lineup, dealer, state, type = 'cashgame') {
    const lineupType = type === 'tournament' ? 'busy' : 'involved';
    ...
    const next = this.nextPlayer(lineup, dealer + 1, lineupType, state);
    const activePlayerCount = countPlayers(lineup, lineupType, state, this.rc);
    if (activePlayerCount < 2) {
   ...
   * @returns
   */
  getBbPos(lineup, dealer, state, type = 'cashgame') {
